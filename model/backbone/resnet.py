...
class B2_ResNet(nn.Module):
    ...
    def initialize_weights(self, path):
        if isinstance(path, str) and path.lower() in ('torchvision', 'imagenet', 'default'):
            weights_enum = getattr(models, 'ResNet50_Weights', None)
            if weights_enum is not None:
                res50 = models.resnet50(weights=weights_enum.IMAGENET1K_V1)
            else:
                res50 = models.resnet50(pretrained=True)
            source = 'torchvision ImageNet weights'
        else:
            res50 = models.resnet50(pretrained=False)
            resnet50_dict = torch.load(path, map_location='cpu')
            if isinstance(resnet50_dict, dict) and 'state_dict' in resnet50_dict:
                resnet50_dict = resnet50_dict['state_dict']
        ...
        print(f'==> Load pretrained ResNet50 parameters from {source}')
